...
/**
 * Queue page component
 * Manages transaction approvals and rejections
 */
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="flex-1">
                      ...
                      {tx.value && (
                        <p className="text-sm text-gray-600 mt-1">
                          Amount: <span className="font-mono font-semibold">{tx.value} {tx.type === 'transfer' ? 'ETH' : ''}</span>
                        </p>
                      )}
                      {tx.recipient && (
                        <p className="text-sm text-gray-600 mt-1">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="space-y-2">
                      {tx.signers.map((signer) => (
                        <div key={signer.signerAddress} className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-gray-700">{signer.signerName}</span>
                          </div>
                          <span className="text-xs font-mono text-gray-500">
                            {signer.status === 'approved' && signer.timestamp
                              ? signer.timestamp.toLocaleDateString()
                              : signer.status}
                          </span>
                        </div>
                      ))}
        ...
        <div>
          ...
          {/* Vault Health */}
          <Card className="p-6 mt-6" style={{ backgroundColor: '#faf5f7', borderColor: '#c99eb8', borderWidth: '1px' }}>
            <h3 className="text-sm font-semibold uppercase tracking-wide mb-4" style={{ color: '#3d1d2c' }}>
              Vault Health
            </h3>
            <div className="space-y-3">
              <div>
                <p className="text-xs font-medium mb-1" style={{ color: '#5a2940' }}>Institutional</p>
